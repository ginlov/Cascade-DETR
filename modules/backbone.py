    ...
    def __init__(self, backbone_name, pretrained=False, batch_norm_freeze=False, resolution_increase=False):
        ...
        if backbone_name == 'resnet50':
            if batch_norm_freeze:
                backbone = resnet50(pretrained=pretrained, norm_layer=FrozenBatchNorm2d, replace_stride_with_dilation=[False, False, True])
            else:
                backbone = resnet50(pretrained=pretrained, replace_stride_with_dilation=[False, False, True])
        elif backbone_name == 'resnet101':
            if batch_norm_freeze:
                backbone = resnet101(pretrained=pretrained, norm_layer=FrozenBatchNorm2d,replace_stride_with_dilation=[False, False, True])
            else:
                backbone = resnet101(pretrained=pretrained, replace_stride_with_dilation=[False, False, True])
        else:
