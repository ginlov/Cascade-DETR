...
def dice_loss(inputs, targets, num_boxes, eps=1e-6):
    """
    Compute the Dice loss, a measure of overlap between two samples.
    """
...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha=0.25, gamma=2.0, eps=1e-6):
    """
    Compute the sigmoid focal loss between `inputs` and the binary `targets`.
    """
    prob = inputs.sigmoid()
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        idx_sorted = torch.argsort(indices[0][1])

        logger.info(f"Indices: {indices[0][1][idx_sorted]}, {indices[0][0][idx_sorted]}")
